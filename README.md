...
- **The installed hook and repository copy can drift.** `~/.claude/hooks/speak-reply.sh` is the runtime source of truth. It currently records `gate_skip` and `queued` events that are absent from `~/speech/hooks/speak-reply.sh`; keep the two copies synchronized when changing hook behavior.
